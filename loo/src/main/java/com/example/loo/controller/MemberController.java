...
import com.example.loo.model.member.MemberJoinForm;
...
public class MemberController {
	...
	public String signUp(@Validated @ModelAttribute("member")
						 MemberJoinForm memberJoinForm, BindingResult result, Model model) {
		
		//회원가입 처리
		log.info("users:{}", memberJoinForm);
		...
		//이메일 주소검증
		if(!memberJoinForm.getCompany_mail().contains("@")) {
			result.reject("emailError", "이메일 형식이 맞지 않습니다.");
			return "users/signup";
		}
		
		//회원가입 중복 확인
		Member checkToEmail = memberMapper.findMember(memberJoinForm.getCompany_mail());
